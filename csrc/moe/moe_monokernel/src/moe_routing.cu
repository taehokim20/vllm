 ...
 *
 * Each thread holds exactly @p Count values.  The compile-time count keeps
 * the per-thread arrays fully unrollable and register-resident (a runtime
 * bound would spill them to local memory).
 */
