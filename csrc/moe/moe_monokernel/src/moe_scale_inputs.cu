...
__device__ inline void routing_phase_quantize(
    ...
    // EP in-kernel dispatch OVERLAP (Stage 2): when `peer_activations` is
    ...
    // Defaults (nullptr/0/0) preserve the original non-EP / M1 behavior.
  ...
  for (std::uint32_t i = w_idx; i < PAIRS_TOTAL; i += NUM_QUANT_WARPS) {
    ...
    // HIDDEN = K_BLOCKS_TOTAL * K_STEP_WGMMA, so row (token, kblk) starts at
