...
__global__ extern void moe_kernel_topk(
    ...
    // When tp_size > 1, Phase 5 uses the LL protocol to all-reduce
    // across ranks and fuses residual + RMSNorm into the writeback.
    // When tp_size <= 1, these are ignored and Phase 5 does a plain
